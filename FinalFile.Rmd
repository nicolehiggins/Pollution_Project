...
labs(title = "AQI of Indian Cities in 2020")  
  theme_map()
```
US: As the US has a smaller industrial base and tighter environmental regulations (which we’ll get more into later) than India; the areas that have worse air pollution tend to be in the south such as Mississippi, Tennessee, and Texas. The areas with the best air quality tend to be the ones with sparse populations in the west. For example, states in “Big Sky country” such as Wyoming and Montana tend to have AQI levels at or near 0. 
...
US: By looking at the data from 1980, we can see that there used to be a much stronger association between AQI and industry. As shown by the maps below, states typically thought of as having more industry, such as those in the midwest, had higher pollutant concentrations relative to others in 1980 than today. For example look at the decrease in Sulfur Dioxide pollution in Michigan and Indiana since 1980.
...
Mod: Are there any trends you notice that might indicate certain causes for the pollution in these countries?
...
ggmap(india) +
  ...
  theme(plot.title = element_text(face="bold", hjust=0.5))
```
US: One fairly obvious trend in the US with an effect on air pollution is the impact of increasing vehicle fuel efficiency. As shown in the graph below, cars in the US today get many more miles per gallon of gas than they did in 1980. This undoubtedly had a positive effect on AQI as cars, one of the biggest emitters of air pollutants, became cleaner.

```{r}

```

Mod: Do environmental policies or legislation play a role in the change in air pollution levels over the years you analysed?

USA: Government policy definitely plays a big role in the change in US air pollution. The most influential US legislation on air pollution is the 1963 Clean Air act, which was amended and expanded in 1990. A study by researchers at UC Berkeley found that the clean air act was responsible for 60% of the drop in pollution from manufacturing in the US from 1990 and 2008. On the flipside, as alluded to previously, some of the rise in AQI since 2015 could be attributed to the rollback of almost 30 federal rules by the Trump administration related to air pollution.
